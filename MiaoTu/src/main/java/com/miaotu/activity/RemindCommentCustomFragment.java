...
public class RemindCommentCustomFragment extends BaseFragment implements View.OnClickListener{
    private SwipeMenuListView lv;
    private List<RemindLikeCustom> remindLikes;
    private RemindLikeCustomListAdapter adapter;
    private View root;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        root = inflater.inflate(R.layout.fragment_remind_comment_custom,null);
        findView();
        bindView();
        init();
        return root;
    ...
    public void getRemindList(){
        new BaseHttpAsyncTask<Void, Void, RemindLikeCustomResult>(getActivity(), true){
            ...
            protected RemindLikeCustomResult run(Void... params) {
                return HttpRequestUtil.getInstance().getRemindCommentCustomTourList(readPreference("token"));
            }
        }.execute();
    }
    /**
