...
public class RemindLikeCustomFragment extends BaseFragment implements View.OnClickListener{
    ...
    private void init(){
        ...
        adapter = new RemindLikeCustomListAdapter(getActivity(), remindLikes, 1);
    ...
    public void getRemindList(){
        new BaseHttpAsyncTask<Void, Void, RemindLikeCustomResult>(getActivity(), true){
            ...
            protected RemindLikeCustomResult run(Void... params) {
                return HttpRequestUtil.getInstance().getRemindLikeCustomList(readPreference("token"));
            }
        }.execute();
    }
    /**
